...
public class ConditionServiceTest {
    ...
    public void testUpdateCondition() {
        ...
        verify(cache, times(1)).put(existingCondition.getId().toString(), updatedCondition);
    }
    ...
    public void testGetConditionByIdWithNonExistingIdThrowsBadRequestException() {
        ...
        assertThrows(BadRequestException.class, () -> conditionService.getConditionById(id));
        verify(conditionRepository, never()).findById(id);
        verify(cache, never()).put(anyString(), any(Condition.class));
    }
    ...
    public void testGetAllConditions() {
        ...
        assertEquals(conditions, allConditions);
        verify(conditionRepository, times(1)).findAll();
        verify(cache, times(2)).put(anyString(), any(Condition.class));
    }
    ...
    @Test
    public void testGetConditionByTextWithNonExistingTextThrowsBadRequestException() {
        String text = "Sunny";

        when(conditionRepository.findByText(text)).thenReturn(null);

        assertThrows(BadRequestException.class, () -> conditionService.getConditionByText(text));
        verify(conditionRepository, times(1)).findByText(text);
        verify(cache, never()).put(anyString(), any(Condition.class));
    }
}
